  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <TradingViewLink symbol={selected.symbol} variant="hero" label={`Open SET:${selected.symbol} in TradingView`} />
              <button onClick={() => setChartMode("tradingview")} className="inline-flex items-center justify-center rounded-2xl border border-blue-200 bg-blue-50 px-5 py-3 text-sm font-bold text-blue-700 hover:bg-blue-100">Show TradingView below</button>
            </div>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <MetricCard icon="building" label="Fundamental" value={selected.fundamental} sub="Manual score / business quality" />
        <MetricCard icon="barChart" label="Technical" value={selected.technical} sub="MA trend, RSI, momentum" />
        <MetricCard icon="dollar" label="Valuation" value={selected.valuation} sub="Manual score / valuation risk" />
        <MetricCard icon="newspaper" label="Impact" value={selected.impact} sub="Calculated from factors.json" />
      </div>

      <Card>
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Score Logic</h2>
            <p className="mt-1 text-sm text-slate-500">Total = Fundamental 35% + Technical 30% + Valuation 15% + Impact 20%</p>
          </div>
          <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{selected.scoreVersion || "v1"}</span>
        </div>
        <div className="mt-4 grid gap-3 md:grid-cols-4">
          {[
            ["Fundamental", selected.fundamental, "Manual"],
            ["Technical", selected.technical, "Auto"],
            ["Valuation", selected.valuation, "Manual"],
            ["Impact", selected.impact, "Factors"],
          ].map(([label, value, tag]) => (
            <div key={label} className="rounded-2xl bg-slate-50 p-4">
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold text-slate-700">{label}</p>
                <span className="rounded-full bg-white px-2 py-0.5 text-xs font-medium text-slate-500">{tag}</span>
              </div>
              <p className="mt-2 text-2xl font-bold text-slate-950">{value ?? "-"}</p>
            </div>
          ))}
        </div>
        {selected.scoreReason && <p className="mt-4 text-sm text-slate-600">{selected.scoreReason}</p>}
      </Card>
        ...
        <Card className="lg:col-span-2">
          <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs font-medium">
              ...
              <TradingViewLink symbol={selected.symbol} variant="compact" label="Open full chart" />
            </div>
          </div>
          {chartMode === "internal" ? (
